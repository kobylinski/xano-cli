...
import type { RequestHistoryItem, XanoApiMiddleware } from '../types.js'

import { BaseApi } from './base.js'
import { apiRequest, ApiResponse } from './request.js'

export class MiddlewareApi extends BaseApi {
  async create(xanoscript: string): Promise<ApiResponse<XanoApiMiddleware>> {
    return apiRequest(
      this.profile,
      'POST',
  ...
  async get(id: number): Promise<ApiResponse<XanoApiMiddleware>> {
    return apiRequest(
      this.profile,
      'GET',
  ...
  async list(page = 1, perPage = 100): Promise<ApiResponse<{ items: XanoApiMiddleware[] }>> {
    return apiRequest(
      this.profile,
      'GET',
  ...
  async update(id: number, xanoscript: string): Promise<ApiResponse<XanoApiMiddleware>> {
    return apiRequest(
      this.profile,
      'PUT',
      `/api:meta/workspace/${this.workspaceId}/middleware/${id}?${this.branchParam}`,
