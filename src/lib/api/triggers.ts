/**
 * Table Triggers API module
 */

import type { RequestHistoryItem, XanoApiTableTrigger } from '../types.js'

import { BaseApi } from './base.js'
import { apiRequest, ApiResponse } from './request.js'

export class TriggersApi extends BaseApi {
  async create(xanoscript: string): Promise<ApiResponse<XanoApiTableTrigger>> {
    return apiRequest(
      this.profile,
      'POST',
  ...
  async get(id: number): Promise<ApiResponse<XanoApiTableTrigger>> {
    return apiRequest(
      this.profile,
      'GET',
  ...
  async list(page = 1, perPage = 100): Promise<ApiResponse<{ items: XanoApiTableTrigger[] }>> {
    return apiRequest(
      this.profile,
      'GET',
  ...
  async update(id: number, xanoscript: string): Promise<ApiResponse<XanoApiTableTrigger>> {
    return apiRequest(
      this.profile,
      'PUT',
      `/api:meta/workspace/${this.workspaceId}/table/trigger/${id}?${this.branchParam}`,
