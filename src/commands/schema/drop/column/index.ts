...
export default class SchemaDropColumn extends Command {
  ...
  static description = 'Drop a column from a table schema'
  static examples = [
    '<%= config.bin %> schema drop column users old_field --force',
    '<%= config.bin %> schema drop column users temp_column --dry-run',
    '<%= config.bin %> schema drop column 271 unused_field --force',
  ]
  static flags = {
    ...
    'no-sync': Flags.boolean({
      default: false,
      description: 'Skip syncing XanoScript after deletion',
    }),
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(SchemaDropColumn)
    ...
    const column = currentSchema[columnIndex]

    if (column.name === 'id') {
      this.error('Cannot drop primary key column \'id\'')
    }

    if (flags['dry-run']) {
      this.log(`Would drop column '${column.name}' from table '${tableName}'`)
      this.log('')
      this.log('Column details:')
      this.log(`  Name: ${column.name}`)
      this.log(`  Type: ${column.type}${column.nullable ? '?' : ''}`)
      if (column.description) this.log(`  Description: ${column.description}`)
      if (column.default) this.log(`  Default: ${column.default}`)
      if (column.tableref_id) this.log(`  Foreign key: table:${column.tableref_id}`)
      return
    }

    if (!flags.force) {
      this.log(`About to drop column '${column.name}' from table '${tableName}'`)
      this.log('')
      this.log('Column details:')
      this.log(`  Name: ${column.name}`)
      this.log(`  Type: ${column.type}${column.nullable ? '?' : ''}`)
      ...
      const confirmed = await this.confirm('This will permanently drop the column and all its data. Continue?')
      if (!confirmed) {
        this.log('Cancelled.')
        return
      }
    ...
    const newSchema = currentSchema.filter((_, i) => i !== columnIndex)

    const response = await api.replaceTableSchema(tableId, newSchema)

    if (!response.ok) {
      this.error(`Failed to drop column '${args.column}':\n  ${response.error}`)
    }
    ...
    if (flags.json) {
      ...
      this.log(`Dropped column '${args.column}' from table '${tableName}'`)
    }
